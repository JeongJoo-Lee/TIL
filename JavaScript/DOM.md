...
# ☑️ 돔의 조각 DocumentFragment

## 💬 정리
* DocumentFragment 노드는 오직 메모리상에만 존재하는 경량화된 DOM이다.
* DocumentFragment를 DOM노드에 추가한다고 해도 DocumentFragment 노드는 추가되지 않고 그 자식 노드만 추가된다.
* DocumentFragment를 DOM에 추가할 때, DocumentFragment의 자식노드는 더이상 생성한 메모리상의 위치에 존재하지 않는다. 만약 이를 유지하고 싶다면 cloneNode를 통해 복제하는 방법이 있다.
